...
public class CookieAuthReqRepo
    ...
    public void saveAuthorizationRequest(OAuth2AuthorizationRequest authorizationRequest,
        ...
        if(authorizationRequest != null) {
            CookieManager.addCookie(response, AuthorizationCookieName,
                    CookieManager.serialize(authorizationRequest), cookieExpiredSeconds);
        }
        String redirectURIAfterLogin = request.getParameter(RedirectCookieName);
        if(redirectURIAfterLogin != null && !redirectURIAfterLogin.isBlank()) {
            CookieManager.addCookie(response, RedirectCookieName, redirectURIAfterLogin,
