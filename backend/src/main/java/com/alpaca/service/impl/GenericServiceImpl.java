...
/**
 * Abstract service implementation providing common CRUD operations. This class serves as a base for
...
public abstract class GenericServiceImpl<T, ID> implements IGenericService<T, ID> {
  ...
  public List<T> findAllByIds(Collection<ID> ids) {
    ...
    if (!existsAllByIds(ids))
      throw new NotFoundException(String.format("Some %s(s) cannot be found", getEntityName()));
        ...
        .orElseThrow(
            () ->
                new BadRequestException(
                    String.format(
                        "%s with ID %s cannot be updated", getEntityName(), id.toString())));
  }
  ...
  public Page<T> findAllPage(Pageable pageable) {
    if (pageable == null)
      throw new BadRequestException(String.format("%s(s) Page cannot be created", getEntityName()));
