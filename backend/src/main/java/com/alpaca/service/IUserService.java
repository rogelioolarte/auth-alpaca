...
public interface IUserService extends IGenericService<User, UUID> {
   ...
   *
   * <p>This method should be used only after verifying that no existing user has the same email by
   * calling {@link #existsByEmail(String email)}. If a user with the given email already exists, an
   * exception must be thrown before calling this method.
   *
