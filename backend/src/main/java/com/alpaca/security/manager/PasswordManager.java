...
public class PasswordManager {
    ...
    /**
     * Constructs a {@code PasswordManager} using an application-wide secret ("pepper")
     * loaded from a {@link org.springframework.core.io.Resource}.
     *
     * <p>The underlying {@link Pbkdf2PasswordEncoder} is initialized with the
     ...
     * </ul>
     *
     * <p>The resource location is typically configured via the
     * {@code spring.datasource.secret.key} property.
     *
     * @param secretKeyResource a resource pointing to the file containing the
     *                          application-wide secret ("pepper"); must not be
     *                          {@code null} and must contain a non-empty value
     * @throws IllegalStateException if the resource cannot be read or the secret
     *                               value is empty
     */
     ...
     *
     * <p>The resource is expected to contain a single line with the raw secret
     * value. The content is read using UTF-8, trimmed to
     * remove surrounding whitespace, and validated to ensure it is not empty.
     *
