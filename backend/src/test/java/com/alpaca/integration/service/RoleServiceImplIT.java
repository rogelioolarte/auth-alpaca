...
class RoleServiceImplIT {
    ...
    void getUserRolesCaseTwo() {
        Role role = service.save(new Role(secondEntity.getRoleName(),
                secondEntity.getRoleDescription(), new HashSet<>()));
    ...
    void findByRoleNameCaseFour() {
        Role role = service.save(new Role(firstEntity.getRoleName(),
                firstEntity.getRoleDescription(), new HashSet<>()));
