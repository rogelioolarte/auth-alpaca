...
class RoleControllerTest {
  ...
  static final List<Role> listEntities = RoleProvider.listEntities();
  static final RoleResponseDTO firstResponse = RoleProvider.singleResponse();
  static final Role singleEntity = RoleProvider.singleEntity();
  static final RoleRequestDTO singleRequest = RoleProvider.singleRequest();

  private void mockMapperAndServiceForSave() {
    when(mapper.toEntity(
        argThat(r -> r != null &&
            r.getRoleName().equals(singleRequest.getRoleName()) &&
            r.getRoleDescription().equals(singleRequest.getRoleDescription()))))
  ...
  void updateByIdNotFound() throws Exception {
    UUID id = firstResponse.id();
    when(mapper.toEntity(
        argThat(r -> r.getRoleName().equals(singleRequest.getRoleName()))))
        .thenReturn(singleEntity);
  ...
  void findAllPageReturnsPagedList() throws Exception {
    ...
    mockMvc.perform(
            get("/api/role/all-page?page=0&size=10")
