...
class JJwtManagerTest {
  ...
  void setUp() throws Exception {
    ...
    jwtManager =
        new JJwtManager(
            base64PrivateKey, base64PublicKey, ISSUER, String.valueOf(EXPIRATION_MILLIS));
  }
  ...
  void createTokenProducesValidJwt() {
    ...
    UserPrincipal principal =
        new UserPrincipal(userId, profileId, advertiserId, username, null, authorities, null);
    ...
    assertEquals(userId.toString(), claims.get("userId", String.class), "userId claim must match");
    assertEquals(
        profileId.toString(), claims.get("profileId", String.class), "profileId claim must match");
    assertEquals(
  ...
  void manageAuthenticationReturnsValidAuthentication() {
    ...
    assertTrue(
        auth.isAuthenticated(), "Authentication should be marked as authenticated by default");
  ...
  void isValidTokenFalseWhenExpirationInPast() {
    ...
    assertFalse(
        jwtManager.isValidToken(claims), "Should return false if the expiration is in the past");
  }
  ...
  void isValidTokenFalseWhenSubjectBlankOrNull() {
    ...
    assertFalse(
        jwtManager.isValidToken(noSub), "Must return false if the subject (username) is null");
  ...
  void isValidTokenFalseWhenAuthoritiesBlankOrNull() {
    ...
    assertFalse(jwtManager.isValidToken(blankAuth), "Should return false if authorities is blank");
  }

  @Test
  @DisplayName(
      "isValidToken returns true when all required fields are set and expiration is in the future")
  void isValidTokenTrueForValidClaims() {
    ...
    assertTrue(
        ...
        "Should return true if expiration > now, subject, userId and authorities are correctly"
            + " set");
  }
}
