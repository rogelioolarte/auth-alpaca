...
class AuthSuccessHandlerTest {
  ...
  private static class TestableHandler extends AuthSuccessHandler {
    TestableHandler(JJwtManager jwtManager, CookieAuthReqRepo repository, List<URI> redirectUris) {
  ...
  class DetermineTargetUrlTests {
    ...
    @Test
    @DisplayName("Throws UnauthorizedException if no redirect cookie and default URI unauthorized")
    void noCookie_defaultUriUnauthorized() {
      TestableHandler handler =
          new TestableHandler(jwtManager, repository, List.of(URI.create("http://example.com")));
      when(jwtManager.createToken(principal)).thenReturn("tok");
      try (MockedStatic<CookieManager> cm = mockStatic(CookieManager.class)) {
        cm.when(() -> CookieManager.getCookie(request, CookieAuthReqRepo.RedirectCookieName))
    ...
    void validCookie_authorizedUri() {
      TestableHandler handler =
          new TestableHandler(jwtManager, repository, List.of(URI.create("http://localhost")));
  ...
  class OnAuthSuccessTests {
    ...
    void redirectWhenNotCommitted() throws IOException {
      ...
      try (MockedStatic<CookieManager> cm = mockStatic(CookieManager.class)) {
        cm.when(() -> CookieManager.getCookie(request, CookieAuthReqRepo.RedirectCookieName))
            .thenReturn(Optional.of(new Cookie(CookieAuthReqRepo.RedirectCookieName, cookieUrl)));
    ...
    void exceptionWhenUnauthorized() throws IOException {
      ...
      try (MockedStatic<CookieManager> cm = mockStatic(CookieManager.class)) {
        cm.when(() -> CookieManager.getCookie(request, CookieAuthReqRepo.RedirectCookieName))
            .thenReturn(
                Optional.of(new Cookie(CookieAuthReqRepo.RedirectCookieName, "http://bar")));
        assertThrows(
            UnauthorizedException.class,
            () -> handler.onAuthenticationSuccess(request, response, authentication));
