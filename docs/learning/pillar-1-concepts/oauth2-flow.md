...
1. **The Request**: Alice clicks "Connect Google Calendar". Auth Alpaca redirects her to Google's login page. Along with her, it sends a `client_id` (who is asking?) and a [`scope`](../glossary.md#scope) (what exactly do they want to access?).
...
5. **The Prize (The Token)**: Google verifies the code and the secret. If everything matches, it issues an [**Access Token**](../glossary.md#access-token).
...
| ← [Previous](../index.md) | [↑ Learning Index](../index.md) | [Next](jwt-deep-dive.md) → |
|:---|:---:|---:|
