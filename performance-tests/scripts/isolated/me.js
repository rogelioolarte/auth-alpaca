...
export default function () {
  ...
  // Need a token. In isolated tests, we perform a login first.
  ...
  const res = http.get('http://localhost:8080/api/auth/me', {
    headers: { 'Authorization': `Bearer ${token}` }
  });

  check(res, { 'get me success': (r) => r.status === 200 });
  sleep(1);
}

export function handleSummary(data) {
  return {
    'stdout': generateMarkdownSummary('Get Me', data),
  };
}
